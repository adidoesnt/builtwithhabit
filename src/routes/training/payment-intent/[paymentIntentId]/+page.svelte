<script lang="ts">
	...
	$effect(() => {
		const interval = setInterval(() => {
			fetch(`/training/payment-intent/${paymentIntentId}/status`)
				.then((res) => res.json())
				.then((data) => {
					const { status } = data;
...
<div class="bg-beige font-body flex h-[100dvh] w-screen items-center justify-center px-4 py-8">
	<div class="flex flex-col items-center justify-center">
		<div
			class="flex h-[100dvh] w-full flex-col gap-4 justify-center items-center rounded-lg border border-gray-100 bg-white p-8 shadow-lg md:h-fit md:w-fit"
		>
			{#if isSuccess}
