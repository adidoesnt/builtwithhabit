<script lang="ts">
	...
	function handleDirectoryClick(directory: { name: string; url: string }) {
		goto(`/training/trainers/${trainerId}/clients/${clientId}/files/${directory.name}`);
	}
	...
	const getPresignedUrl = async (fileName: string) => {
		switch (currentDir) {
			case UserDir.MEDIA:
				return await fetch(
					`/training/trainers/${trainerId}/clients/${clientId}/files/media/presigned-url?fileName=${fileName}`
				)
					.then((res) => res.json())
					.then((data) => data.presignedUrl);
			case UserDir.TRAINER_NOTES:
				return await fetch(
					`/training/trainers/${trainerId}/clients/${clientId}/files/trainer-notes/presigned-url?fileName=${fileName}`
				)
					.then((res) => res.json())
					.then((data) => data.presignedUrl);
			default:
