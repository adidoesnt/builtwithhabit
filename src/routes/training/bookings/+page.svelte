<script lang="ts">
	...
	$effect(() => {
		isLoading = true;
		fetch(`/training/bookings?page=${page}&pageSize=${itemsPerPage}`)
			.then((res) => res.json())
			.then((data) => {
				bookings = data.items;
			})
...
<div class="bg-beige font-body text-dark-brown min-h-[100dvh] p-4 md:p-8 w-full max-w-[100dvw]">
	<div class="mx-auto max-w-6xl">
		<div class="mt-6 md:mb-8">
			...
			{#if !isTrainer}
				...
				<p class="text-light-brown mt-2 hidden text-sm md:block">
					To make a new booking, <a href="/training/packages" class="text-dark-brown underline">click here</a
					>.
				</p>
			{/if}
		</div>
		...
		<p class="text-light-brown text-sm md:hidden">
			To make a new booking, <a href="/training/packages" class="text-dark-brown underline"
				>click here</a
			>.
		</p>
	</div>
