<script lang="ts">
	const { isOpen, setIsOpen, handleDeleteBookingNotes } = $props();
	...
	async function handleDelete() {
		isDeleting = true;
		try {
			await handleDeleteBookingNotes();
...
{#if isOpen}
	...
	>
		<div class="w-full max-w-sm overflow-hidden rounded-lg bg-white shadow-lg">
			<div class="p-4">
				<h3 class="font-body text-dark-brown text-lg font-semibold">Delete Notes</h3>
				{#if message}
					<div
