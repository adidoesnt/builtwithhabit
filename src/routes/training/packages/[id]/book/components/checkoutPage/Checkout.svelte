<script lang="ts">
	...
	const price = $derived(parseFloat(fetchedPackage.price));
	const amount = $derived(price * 100);
	...
	const fetchClientSecret = async () => {
		const result = await fetch('/training/payment-intent', {
			method: 'POST',
	...
	const updatePurchase = async (paymentIntentId: string) => {
		const result = await fetch(`/training/purchases/${$purchaseId}`, {
			method: 'PUT',
	...
	const initializeStripe = async () => {
		...
		try {
			...
			redirectUrl = `/training/payment-intent/${paymentIntentId}`;
