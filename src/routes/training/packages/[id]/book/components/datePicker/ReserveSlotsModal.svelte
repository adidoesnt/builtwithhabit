<script lang="ts">
	...
	const reserveSelectedSlots = async () => {
		try {
			...
			const response = await fetch('/training/purchases', {
				method: 'POST',
				body: JSON.stringify({
					packageId,
					address: $address,
...
{#if isOpen}
	<div class="fixed z-50 m-0 flex h-[100dvh] w-screen items-center justify-center backdrop-blur-xl">
		<div
			class="flex h-[100dvh] w-full flex-col items-center justify-center gap-4 rounded-lg bg-white p-4 shadow-xl md:h-fit md:w-fit"
		>
