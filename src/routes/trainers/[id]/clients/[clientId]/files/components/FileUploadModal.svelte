<script lang="ts">
	const { isOpen, onClose, getPresignedUrl } = $props();
		...
		try {
			const presignedUrls = await Promise.all(
				selectedFiles.map(async (file) => {
					...
					return { fileName: file.name, presignedUrl };
				})
			);
	...
	>
		<div class="w-full max-w-md overflow-hidden rounded-lg bg-white shadow-md">
			<div class="flex items-center justify-between border-b border-gray-200 bg-gray-50 p-4">
				<h2 class="font-body text-dark-brown text-lg font-semibold">Upload Files</h2>
				<button
			...
			<form class="p-4">
				...
				{/if}
				<div class="mb-4">
					<label class="font-body text-dark-brown mb-2 block text-sm font-medium" for="file-upload">
					...
					>
						<div class="text-center">
							{#if isLoading}
								...
								<p class="font-body text-light-brown mt-2 text-sm">Uploading files...</p>
							{:else}
								<svg
									xmlns="http://www.w3.org/2000/svg"
					...
					{#if selectedFiles.length > 0}
						<div class="mt-4">
							<p class="font-body text-dark-brown mb-2 text-sm font-medium">Selected files:</p>
							<ul class="max-h-32 overflow-y-auto">
								{#each selectedFiles as file}
									<li class="font-body text-light-brown text-sm">{file.name}</li>
								{/each}
							</ul>
						</div>
					{/if}
